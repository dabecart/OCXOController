...
void drawPixelTFT(TFT* tft, uint16_t x, uint16_t y, uint16_t color) {
    ...
    setAddressWindowTFT_(tft, x, y, x+1, y+1);
...
void writeCharTFT_(TFT* tft, uint16_t x, uint16_t y, char ch, FontDef font, uint16_t color, uint16_t bgcolor) {
    ...
    setAddressWindowTFT_(tft, x, y, x+font.width-1, y+font.height-1);
...
void writeStringTFT(TFT* tft, uint16_t x, uint16_t y, const char* str, FontDef font, uint16_t color, uint16_t bgcolor) {
    ...
    while(*str) {
        if(x + font.width >= tft->width) {
            x = 0;
            y += font.height;
            if(y + font.height >= tft->height) {
                break;
            }
        ...
        writeCharTFT_(tft, x, y, *str, font, color, bgcolor);
        x += font.width;
        str++;
    }
...
void fillRectangleTFT(TFT* tft, uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color) {
    ...
    selectTFT_(tft);
    setAddressWindowTFT_(tft, x, y, x+w-1, y+h-1);

    uint8_t data[] = {(uint8_t)(color >> 8), (uint8_t)(color & 0xFF)};
    HAL_GPIO_WritePin(TFT_A0_GPIO_Port, TFT_A0_Pin, GPIO_PIN_SET);
    for(y = h; y > 0; y--) {
        for(x = w; x > 0; x--) {
            HAL_SPI_Transmit(tft->hspi, data, sizeof(data), HAL_MAX_DELAY);
        }
    }
...
void fillScreenTFT(TFT* tft, uint16_t color) {
    fillRectangleTFT(tft, 0, 0, tft->width, tft->height, color);
}

void drawImageTFT(TFT* tft, uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint16_t* data) {
    if((x >= tft->width) || (y >= tft->height)) return;
    if((x + w - 1) >= tft->width) return;
    if((y + h - 1) >= tft->height) return;

    selectTFT_(tft);
    setAddressWindowTFT_(tft, x, y, x+w-1, y+h-1);
    writeDataTFT_(tft, (uint8_t*)data, sizeof(uint16_t)*w*h);
    unselectTFT_(tft);
}
